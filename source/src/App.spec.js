...
const server = setupServer(
    ...
    // ставим запрос с UserList иначе посыпятся warnings
...
describe('Routing', () => {
  ...
  `(
        ...
        ({path, pageTestId}) => {
            setup(path);
            const page = screen?.queryAllByTestId(pageTestId);
    ...
    it.each`
        ...
        `('displays $visiblePage after clicking $clickingTo', ({initialPath, clickingTo, visiblePage}) => {
        setup(initialPath);
    ...
    describe("Login", () => {
        ...
        const setupLoggedIn = () => {
            ...
            userEvent.type(inputEmail, 'user5@email.com');
        ...
        it('stores logged state in local storage', async () => {
            setupLoggedIn();
            const page = await screen.findByTestId('home-page');
        ...
        it('refresh user page from another user to the logged in user after clocking My Profile', async () => {
            storage.setItem('auth', {
                id: 5,
                username: 'user5',
                isLoggedIn: true
